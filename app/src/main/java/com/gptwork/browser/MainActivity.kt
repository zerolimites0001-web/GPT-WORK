...
class MainActivity : AppCompatActivity() {
    ...
    private fun buildUi() {
        ...
        address = EditText(this).apply { hint="Search or enter address"; singleLine=true; setTextColor(Color.WHITE); setHintTextColor(Color.LTGRAY); setBackgroundColor(Color.rgb(30,32,38)); setPadding(18,0,18,0); setOnEditorActionListener { _,_,_ -> navigateInput(); true } }
        bar.addView(address, LinearLayout.LayoutParams(0,52,1f))
        bar.addView(button("↻") { web.reload() })
        tabs = button("1") { showMenu(it) }
        bar.addView(tabs)
        root.addView(bar)
        web = WebView(this)
    ...
    private fun newTab(url: String) { pages.add(url); active=pages.lastIndex; web.loadUrl(url); tabs.text=pages.size.toString() }
    ...
    private fun showMenu(anchor: View) {
        ...
        menu.menu.add("Reload").setOnMenuItemClickListener { web.reload(); true }
        menu.show()
    }

    private fun applyTheme() {
        val bg=if(dark) Color.rgb(16,17,20) else Color.WHITE
        window.statusBarColor=bg; window.navigationBarColor=bg
        address.setTextColor(if(dark) Color.WHITE else Color.DKGRAY)
    }
